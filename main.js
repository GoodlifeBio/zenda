...
(function () {
  ...
  function initFallingPattern() {
    var patternEl = document.getElementById('falling-pattern');
    var overlayEl = document.getElementById('falling-pattern-overlay');
    if (!patternEl || !overlayEl) return;

    const style = getComputedStyle(document.documentElement);
    const duration = parseFloat(style.getPropertyValue('--falling-duration')) || 150;
    const blurIntensity = style.getPropertyValue('--blur-intensity') || '10px';
    const density = parseFloat(style.getPropertyValue('--density')) || 1;
    ...
    patternEl.style.backgroundImage = patterns.join(', ');
    patternEl.style.backgroundSize = backgroundSizes;

    // Overlay Dot Grid
    overlayEl.style.backgroundImage = `radial-gradient(circle at 50% 50%, transparent 0, transparent 2px, ${backgroundColor} 2px)`;
    overlayEl.style.backgroundSize = `${8 * density}px ${8 * density}px`;

    // Animation via GSAP
    gsap.fromTo(patternEl, 
