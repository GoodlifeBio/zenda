...
class AnimationController {
    ...
    public render() {
        ...
        // Let's support light/dark mode by drawing transparently or using body classes
        // In the original, it forces 'black' rect. Let's make it transparent to overlay correctly.
        ctx.clearRect(0, 0, this.size, this.size)
        // Note: the component wrapper requires a background, see the wrapper container.
        ...
        // We will adapt color to light/dark mode by pulling from document element
        ...
        const starColor = isDark ? 'white' : 'rgba(30,30,30,0.4)';

        // 绘制轨迹
        this.drawTrail(t1, starColor)
    ...
    private drawTrail(t1: number, color: string) {
        for (let i = 0; i < this.trailLength; i++) {
            ...
            this.ctx.beginPath()
            this.ctx.arc(rotated.x, rotated.y, Math.max(0.2, sw / 2), 0, Math.PI * 2)
            this.ctx.fill()
        }
    }
...
class Star {
    ...
    render(p: number, controller: AnimationController) {
        ...
        if (q > 0) {
            ...
            // 将2D屏幕坐标转换为3D空间坐标
            // Need to hack typing for private properties
            const camZ = (controller as any)['cameraZ'];
            const viewZoom = (controller as any)['viewZoom'];

            const vx = (this.z - camZ) * screenX / viewZoom;
            const vy = (this.z - camZ) * screenY / viewZoom;
...
export function SpiralAnimation() {
    ...
    useEffect(() => {
        ...
        // 创建动画控制器 (if not exists)
        if (!animationRef.current) {
            animationRef.current = new AnimationController(canvas, ctx, dpr, size)
        } else {
            // Update context if resized
            (animationRef.current as any).ctx = ctx;
            (animationRef.current as any).size = size;
        }
